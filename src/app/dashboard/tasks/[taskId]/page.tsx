  ...
  if (files.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center dark:border-slate-700 dark:bg-slate-900/70">
        <h3 className="text-lg font-semibold text-slate-950 dark:text-white">No assignment files yet</h3>
...
function SubmissionHistory({ submissions }: { submissions: TaskSubmission[] }) {
  if (submissions.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center dark:border-slate-700 dark:bg-slate-900/70">
        <h3 className="text-lg font-semibold text-slate-950 dark:text-white">No staff submissions yet</h3>
        <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">
...
export default async function DashboardTaskDetailPage({
  ...
  const managedDepartmentIds = isDepartmentAdmin
    ...
    : [];
  const scopedDepartments = isDepartmentAdmin
    ? await getDepartments(managedDepartmentIds).catch(() => [])
    : [];
  const visibleDepartments = scopedDepartments.map((department) => department.name);
  const detail = await getTaskDetailById(taskId, isDepartmentAdmin ? visibleDepartments : undefined);
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-950 p-5 text-white shadow-[0_24px_90px_rgba(15,23,42,0.22)] lg:p-7">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              ...
              <Link
                href={`/dashboard/workflow?edit=${detail.task.id}`}
            ...
            <section className="rounded-3xl border border-white/80 bg-white/90 p-6 shadow-[0_20px_70px_rgba(15,23,42,0.08)] backdrop-blur dark:border-slate-800 dark:bg-slate-950/80">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
                  ...
                  <p className="mt-2 text-sm font-semibold text-slate-950 dark:text-white">{detail.task.assigneeName}</p>
                  <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{detail.task.assigneeEmail || "No email available"}</p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                    Reviewer
            ...
            {latestPendingSubmission ? (
              <form action={reviewTaskSubmissionAction} className="mt-6 space-y-5">
                ...
                <label className="block space-y-2">
                  <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Reviewer Note</span>
                  <textarea
                    name="reviewNote"
                    rows={8}
              ...
              <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm leading-6 text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400">
